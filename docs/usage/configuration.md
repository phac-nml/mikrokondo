...
VAR_NAME { // Replace VAR name with the genus name of your sample, only use ASCII (a-zA-Z) alphabet characters in the name and replace spaces, punctuation and other special characters with underscores (_)
    ...
    staramr_percent_length_overlap_resfinder = // Set the percent length overlap for resfinder results, default is 60
    staramr_percent_length_overlap_pointfinder = // Set the percent length overlap for pointfinder results, default is 95
    staramr_genome_size_lower_bound = min_length // Keep these the same as other genome length unless you want specific staramr QC metrics
    staramr_genome_size_upper_bound = max_length // Keep these the same as other genome length unless you want specific staramr QC metrics
}
```
...
salmonella {
    ...
    IDTool = "SISTR"
    staramr_percent_length_overlap_resfinder = 52
    staramr_percent_length_overlap_pointfinder = 95
    staramr_genome_size_lower_bound = min_length
    staramr_genome_size_upper_bound = max_length
}
```
...
#### StarAMR parameter configuration:

StarAMR has four parameters that are set within the QCReport configuration:
...
StarAMR specific settings are linked to the pointfinder database used. The accepted pointfinder databases are ('salmonella', 'campylobacter', 'enterococcus_faecalis','enterococcus_faecium', 'escherichia_coli', 'helicobacter_pylori') using the `starmar.point_finder_dbs` parameter.

```
point_finder_dbs = [
        'salmonella': [params.QCReport.salmonella], 
        'campylobacter': [params.QCReport.campylobacter_jejuni, params.QCReport.campylobacter_coli, params.QCReport.campylobacter] 
        'enterococcus_faecalis': [params.QCReport.enterococcus],
        'enterococcus_faecium': [params.QCReport.enterococcus], 
        'escherichia_coli': [params.QCReport.escherichia], 
        'helicobacter_pylori': []
        ]
```  

To have a new configuration included within a pointfinder database use the logic:
