...
I realized I was being algorithmically dense when I wrote the first version of this script and that it can be refactored to use significantly less memory when running.
...
class FastQData:
    ...
    def __init__(self, name: str, numeric_conversion: t.Callable[[int], float|decimal.Decimal]):
        self.name = name
    ...
    def __add__(self, obj: 'FastQData') -> 'FastQData':
        ...
        sequence_lengths = self._combine_dictionaries(self.sequence_lengths, obj.sequence_lengths)
        avg_qual_read = self._combine_dictionaries(self.avg_qual_read, obj.avg_qual_read)
    ...
    def _dict_mean(self, vector: defaultdict[int, int], total: int) -> float | decimal.Decimal:
        """
        ...
        for key, value in vector.items():
            ...
            if key < min_length:
                min_length = key
        return self.numeric_conversion(sum_) / self.numeric_conversion(total), min_length, max_length
    
    def _dict_std(self, vector: defaultdict[int, int], mean: float | decimal.Decimal, total: int):
        """
        ...
        normalized_squares = self.numeric_conversion(sum_of_squares) / self.numeric_conversion(total)
    ...
    def __set_qual_std(self):
        ...
        for key, value in enumerate(self.quality):
            if value != 0:
               ...
               diff_sq = math.pow(diff, 2)
               sum_of_squares += diff_sq * value
        normalized_squares = self.numeric_conversion(sum_of_squares) / self.numeric_conversion(self.total_bp)
        standard_deviation = math.sqrt(normalized_squares)
        return standard_deviation
    ...
    def __set_qual_mean(self):
        ...
        """
        return self.numeric_conversion(self.qual_sum) / self.numeric_conversion(self.total_bp)
    ...
    def _update_quality_vector(self, quality: str):
        ...
        for x in quality:
            ...
            qual_sum += idx
            self.quality[idx] += 1
        read_average_quality = self.numeric_conversion(qual_sum) / self.numeric_conversion(len(quality))
